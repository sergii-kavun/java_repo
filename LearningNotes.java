...
1. BREAK
--Once code reached this operator, it goes out from the loop

2. CONTINUE
-- Once code reached this operator, it stop current iteration of LOOP and start next one

-----------------------------CHAPTER 6 -----------------------------------------------

-------JAVA API-----------
// It is the Java Library with lots of prebuild classes in it to use from the box.
//

1. ArrayList

 ArrayList x = new ArrayList();
        x.add(obj elem);  // adds the object parameter to the list
        x.remove(int index);  // removes object at the index param
        x.remove(object elem);  // removes this object from list
        x.contains(object elem); // returns true if there is a match for the searching param
        x.isEmpty(); // returns true is the list has no elements
        x.indexOf(Object elem); // returns either index of the object or -1
        x.size();  // reutrns the number of elements currently in the list
        x.get(int index); // reutrns object by index
*/