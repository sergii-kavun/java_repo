...
// REAL CODE implementation
    ...
    public String checkYourself (String stringGuess) {
        ...
        for (int i=0; i < locationCells.length; i++ ) { // loop for each element of array we are assigning cell variable
            if (locationCells[i] < 0 ) {
                // if any of element of array is negative it will skip the iteration of LOOP and star new one
                continue;
            }

            if (locationCells[i] == guess  ) {  // compare user guess to the cell element of array
                    result = "hit";
                    numOfHits++;
                    locationCells[i] = -1;  // set hit value to -1. We are replacing guessed value to -1 to not use it further
                    break;
                }
